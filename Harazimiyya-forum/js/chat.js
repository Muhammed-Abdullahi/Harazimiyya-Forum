...
// ================= WHATSAPP-STYLE MEDIA FUNCTIONS =================

// Store downloaded media URLs to remember they've been downloaded
let downloadedMedia = new Set();

// Render message content with WhatsApp-style blurry preview
function renderMessageContent(msg) {
    if (msg.message_type === 'text') {
        // CRITICAL FIX: Convert line breaks to <br> tags AND use pre-wrap for display
        var textWithBreaks = (msg.content || '').replace(/\n/g, '<br>');
    ...
    if (msg.message_type === 'image') {
        ...
        // Check if this media has been downloaded before
        var isDownloaded = downloadedMedia.has(msg.id);
        
        if (isDownloaded) {
            // Show clear image
            return '<div class="message-image-clear" onclick="openMediaModal(\'' + msg.id + '\', \'image\', \'' + imageUrl + '\')">' +
                   '<img src="' + imageUrl + '" alt="Image" style="max-width: 100%; cursor: pointer;">' +
                   '</div>';
        } else {
            // Show blurry preview with download overlay
            return '<div class="message-image-blurry" onclick="openMediaModal(\'' + msg.id + '\', \'image\', \'' + imageUrl + '\')">' +
                   '<img src="' + imageUrl + '" alt="Image" style="max-width: 100%;">' +
                   '<div class="download-overlay"><i class="fas fa-download"></i> Tap to download</div>' +
                   '</div>';
        }
    }
    ...
    if (msg.message_type === 'video') {
        ...
        if (isDownloaded) {
            // Show clear video
            return '<div class="message-video-clear" onclick="openMediaModal(\'' + msg.id + '\', \'video\', \'' + videoUrl + '\')">' +
                   '<video><source src="' + videoUrl + '"></video>' +
                   '</div>';
        } else {
            // Show blurry preview with download overlay
            return '<div class="message-video-blurry" onclick="openMediaModal(\'' + msg.id + '\', \'video\', \'' + videoUrl + '\')">' +
                   '<video><source src="' + videoUrl + '"></video>' +
                   '<div class="download-overlay"><i class="fas fa-download"></i> Tap to download & play</div>' +
                   '</div>';
        }
    }
    ...
    if (msg.message_type === 'audio') {
        var audioUrl = safeUrl(msg.file_url);
        if (!audioUrl) return '<p>Audio not available</p>';
        // Audio doesn't need blur - can play directly
        return '<audio controls src="' + audioUrl + '" style="width: 280px; height: 40px;"></audio>';
    }
...
// Open media modal with WhatsApp-style download requirement
window.openMediaModal = function(messageId, mediaType, mediaUrl) {
    console.log("Opening media modal for:", messageId, mediaType);
    
    var modal = document.getElementById('mediaModal');
    var modalBody = document.getElementById('mediaModalBody');
    var modalTitle = document.getElementById('mediaModalTitle');
    var downloadBtn = document.getElementById('downloadMediaBtn');
    
    // Check if already downloaded
    var isDownloaded = downloadedMedia.has(messageId);
    
    // Set title
    modalTitle.innerHTML = mediaType === 'image' ? '<i class="fas fa-image"></i> Image Preview' : '<i class="fas fa-video"></i> Video Preview';
    
    // Create media element (blurry initially)
    if (mediaType === 'image') {
        modalBody.innerHTML = '<img src="' + mediaUrl + '" alt="Image" class="' + (isDownloaded ? 'modal-media-clear' : 'modal-media-blurry') + '" id="modalMedia">';
    } else {
        modalBody.innerHTML = '<video controls class="' + (isDownloaded ? 'modal-media-clear' : 'modal-media-blurry') + '" id="modalMedia"><source src="' + mediaUrl + '"></video>';
    }
    
    // If already downloaded, show clear and disable download button
    if (isDownloaded) {
        downloadBtn.innerHTML = '<i class="fas fa-check"></i> Already Downloaded';
        downloadBtn.disabled = true;
        downloadBtn.style.opacity = '0.5';
        downloadBtn.style.cursor = 'not-allowed';
    } else {
        downloadBtn.innerHTML = '<i class="fas fa-download"></i> Download to View Clearly';
        ...
        downloadBtn.onclick = function() {
            downloadMedia(messageId, mediaType, mediaUrl);
        };
    }
...
// Download media function (makes it clear)
function downloadMedia(messageId, mediaType, mediaUrl) {
    console.log("Downloading media:", messageId);
    ...
    setTimeout(function() {
        // Mark as downloaded
        downloadedMedia.add(messageId);
        
        // Make media clear in modal
        modalMedia.classList.remove('modal-media-blurry');
        modalMedia.classList.add('modal-media-clear');
        
        // Update button
        downloadBtn.innerHTML = '<i class="fas fa-check"></i> Downloaded - Viewing Clearly';
        
        // Update the message in the chat to show clear version
        updateMessageToClear(messageId, mediaType, mediaUrl);
        
        // Show notification
        showNotification('✅ Media downloaded and ready to view', 'success', 2000);
        
    }, 1500); // Simulate download time
}

// Update message in chat to clear version
function updateMessageToClear(messageId, mediaType, mediaUrl) {
    var messageEl = document.querySelector('.message[data-message-id="' + messageId + '"]');
    if (!messageEl) return;
    
    var contentWrapper = messageEl.querySelector('.message-content');
    if (!contentWrapper) return;
    
    if (mediaType === 'image') {
        contentWrapper.innerHTML = '<div class="message-image-clear" onclick="openMediaModal(\'' + messageId + '\', \'image\', \'' + mediaUrl + '\')">' +
                                   '<img src="' + mediaUrl + '" alt="Image" style="max-width: 100%; cursor: pointer;">' +
                                   '</div>';
    } else {
        contentWrapper.innerHTML = '<div class="message-video-clear" onclick="openMediaModal(\'' + messageId + '\', \'video\', \'' + mediaUrl + '\')">' +
                                   '<video><source src="' + mediaUrl + '"></video>' +
                                   '</div>';
    }
}
...
// ================= FIXED REPLY FUNCTION (PRESERVES INPUT VISIBILITY) =================
window.showReplyInput = function(parentId, senderName, messageContent, messageType) {
    console.log("📝 Show reply input for message:", parentId);
    ...
    // Show reply preview (input and header ALWAYS stay visible)
    showReplyPreview(senderName, messageContent, messageType);
    
    // Focus input (which is ALWAYS visible)
    var messageInput = document.getElementById('messageInput');
    if (messageInput) {
        messageInput.focus();
        messageInput.classList.add('replying');
        messageInput.placeholder = 'Reply to ' + senderName + '...';
    }
};

// Show reply preview above input (input ALWAYS visible)
function showReplyPreview(senderName, messageContent, messageType) {
    var previewArea = document.getElementById('replyPreviewArea');
    var senderSpan = document.getElementById('replyPreviewSender');
    var messageP = document.getElementById('replyPreviewMessage');
    ...
    senderSpan.textContent = 'Replying to ' + senderName;
    messageP.textContent = previewText;
    
    previewArea.classList.remove('hidden');
}

// Cancel reply (input stays visible)
function cancelReply() {
...
// ================= MODIFIED SEND MESSAGE TO HANDLE REPLY CLEANUP =================
...
async function sendMessage() {
    ...
    try {
        ...
        // Clear reply indicator after sending (input stays visible)
        if (replyToSend) {
            cancelReply();
        }
...
// ================= ENSURE HEADER AND INPUT NEVER DISAPPEAR =================
// Add this to your DOMContentLoaded event
function ensureFixedElements() {
    // Force header and input to always be visible
...
// Call this periodically to ensure elements stay visible
setInterval(ensureFixedElements, 1000);

// Also call on scroll and resize
window.addEventListener('scroll', ensureFixedElements);
window.addEventListener('resize', ensureFixedElements);